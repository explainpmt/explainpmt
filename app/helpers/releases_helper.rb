module ReleasesHelper
  ...
  def link_to_new_release
    link_to 'Add a Release', new_project_release_path(current_project)
  end

  def link_to_edit_release(release, options={})
    link_to (options[:text] || release.name), edit_project_release_path(current_project, release)
  end
  ...
  def option_to_edit_release(release)
    create_action_option("Edit", edit_project_release_path(current_project, release))
  end

  def option_to_delete_release(release)
    create_action_option("Delete", project_release_path(current_project, release), :method => :delete, :confirm => 'Are you sure you want to delete?\r\nAll associated data will also be deleted. This action can not be undone.')
  end
